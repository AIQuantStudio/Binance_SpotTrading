...
class ImprovedPricePredictionLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        h0 = torch.zeros(self.num_layers * 2, x.size(0), self.hidden_size).to("cuda")  # Multiply by 2 for bidirectional
        c0 = torch.zeros(self.num_layers * 2, x.size(0), self.hidden_size).to("cuda")  # Multiply by 2 for bidirectional
